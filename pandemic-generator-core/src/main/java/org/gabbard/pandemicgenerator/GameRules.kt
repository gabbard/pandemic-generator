...
data class GameRules(
    ...
    fun setupGame(rng: Random): GameState {
        ...
        for (player in players) {
            val (cardsDrawn, newDeck) = deckForPlayerHands.draw(initialHandSize(players.size))
        ...
        return GameState(
            ...
            untrackableState = UntrackableState(hands = playerHands, board = initialBoardState)
        )
    }
    ...
    fun createDeckForPlayerHands(rng: Random): Deck<PlayerCard> {
        ...
        return Deck(cityPlayerCards.union(events).toList()).shuffled(rng)
    }
}

fun <T> chooseDistinct(items: Collection<T>, numItemsToChoose: Int, rng: Random): Set<T> {
    require(items.size >= numItemsToChoose) {
        "Cannot choose $numItemsToChoose distinct items from ${items.size} items"
    }
